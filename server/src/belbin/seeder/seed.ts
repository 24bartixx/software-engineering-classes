...
export async function seed(systemConfigService: SystemConfigService) {
  ...
  try {
    ...
    await systemConfigService.set(SystemConfigKeys.REMINDER_COOLDOWN_DAYS, '3');
    await systemConfigService.set(
    ...
    notification.title = await systemConfigService.getOrThrow(
      SystemConfigKeys.EXPIRED_TEST_NOTIFICATION_TITLE,
    );
    const notificationTypeString = await systemConfigService.getOrThrow(
      SystemConfigKeys.EXPIRED_TEST_NOTIFICATION_TYPE,
    );
