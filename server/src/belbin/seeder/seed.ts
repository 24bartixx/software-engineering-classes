...
const employeesData = [
    {
        firstName: 'Jan', lastName: 'Kowalski', email: 'jestgitbartix@gmail.com', gender: 'Male',
        deptIndexes: [0],
        testDate: '2026-01-15',
        belbin: { shaper: 10, implementer: 5, specialist: 2, coordinator: 3, plant: 1, resource: 4, monitor: 2, team: 6, completer: 3 }
    },
    {
        firstName: 'Anna', lastName: 'Nowak', email: 'anna.n@test.com', gender: 'Female',
        deptIndexes: [4],
    },
    {
        firstName: 'Piotr', lastName: 'Wiśniewski', email: 'piotr.w@test.com', gender: 'Male',
        deptIndexes: [2, 5],
    },
    {
        firstName: 'Matthew', lastName: 'Perrxhan', email: 'mati.pxp20@gmail.com', gender: 'Male',
        deptIndexes: [3],
        testDate: '2018-05-20',
        belbin: { shaper: 1, implementer: 2, specialist: 4, coordinator: 9, plant: 8, resource: 7, monitor: 3, team: 9, completer: 2 }
    }
];
...
export async function seed(systemConfigService: SystemConfigService) {
    const AppDataSource = new DataSource({
        ...
        entities: [Address, User, Employee, Department, EmployeeDepartment, BelbinTest],
        synchronize: false,
    });
    ...
    try {
        ...
        const savedDepartments = [
            ...
            await saveDepartment("Sprzedaż", queryRunner),
            await saveDepartment("Marketing", queryRunner),
            await saveDepartment("Finanse", queryRunner),
            await saveDepartment("Obsługa klienta", queryRunner),
            await saveDepartment("Logistyka", queryRunner),
            await saveDepartment("Produkcja", queryRunner),
...
async function saveDepartment(name: string, queryRunner: QueryRunner): Promise<Department> {
