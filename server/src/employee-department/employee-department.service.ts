...
export class EmployeeDepartmentService {
    ...
    async create(createEmployeeDepartmentDto: CreateEmployeeDepartmentDto): Promise<EmployeeDepartment> {
        ...
        await this.ensureEmployeeDepartmentNotExist(createEmployeeDepartmentDto.employeeId, createEmployeeDepartmentDto.departmentId);

        const employeeDepartment = this.employeeDepartmentRepository.create(createEmployeeDepartmentDto);
    ...
    async findOne(employeeId: number, departmentId: number): Promise<EmployeeDepartment> {
        ...
        if (!employeeDepartment) {
            throw new NotFoundException(`EmployeeDepartment with employeeId ${employeeId} and departmentId ${departmentId} not found`);
        }
    ...
    private async ensureEmployeeExists(employeeId: number) {
        const employeeExists = await this.employeeRepository.exists({ where: { id: employeeId } });
        if (!employeeExists) {
    ...
    private async ensureDepartmentExists(departmentId: number) {
        const departmentExists = await this.departmentRepository.exists({ where: { id: departmentId } });
        if (!departmentExists) {
            throw new NotFoundException(`Department with ID ${departmentId} not found`);
        }
    }

    private async ensureEmployeeDepartmentNotExist(employeeId: number, departmentId: number) {
        const existing = await this.employeeDepartmentRepository.findOneBy({
            employeeId: employeeId,
            departmentId: departmentId
        });
        if (existing) {
            throw new BadRequestException('This employee is already assigned to this department (record exists).');
        }
    }
}
